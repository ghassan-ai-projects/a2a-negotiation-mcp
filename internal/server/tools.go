...
import (
	...
	"github.com/ghassan-ai-projects/a2a-negotiation-mcp/internal/backupmgr"
	...
	"github.com/ghassan-ai-projects/a2a-negotiation-mcp/internal/serverconfig"
	...
	"github.com/ghassan-ai-projects/a2a-negotiation-mcp/internal/scheduler"
...
type NegotiationServer struct {
	...
	serverCfgStore      *serverconfig.Store
	backupStore      *backupmgr.Store
	logger                *slog.Logger
...
func (ns *NegotiationServer) registerTools() {
	...
	ns.mcpServer.AddTool(mcp.NewTool("negotiate_export_dashboard",
        ...
        // P101: Scheduled Negotiation Runs
        ns.mcpServer.AddTool(mcp.NewTool("negotiate_schedule_run",
                mcp.WithDescription("Schedule an automatic negotiation run on a recurring basis."),
                mcp.WithString("vendor", mcp.Required(), mcp.Description("Vendor name")),
                mcp.WithString("strategy", mcp.Required(), mcp.Description("Negotiation strategy (aggressive, balanced, conservative)")),
                mcp.WithString("cron_expr", mcp.Required(), mcp.Description("Cron expression for scheduling (e.g. 0 */6 * * * for every 6 hours)")),
        ), ns.handleScheduleRun)

        ns.mcpServer.AddTool(mcp.NewTool("negotiate_list_schedules",
                mcp.WithDescription("List all scheduled negotiation runs."),
        ), ns.handleListSchedules)

        ns.mcpServer.AddTool(mcp.NewTool("negotiate_delete_schedule",
                mcp.WithDescription("Delete a scheduled negotiation run."),
                mcp.WithInteger("schedule_id", mcp.Required(), mcp.Description("Schedule ID to delete")),
        ), ns.handleDeleteSchedule)

        ns.mcpServer.AddTool(mcp.NewTool("negotiate_schedule_results",
                mcp.WithDescription("View results of a scheduled negotiation run."),
                mcp.WithInteger("schedule_id", mcp.Required(), mcp.Description("Schedule ID to view results for")),
        ), ns.handleScheduleResults)

	// Tool: negotiate_create_workflow
	ns.mcpServer.AddTool(mcp.NewTool("negotiate_create_workflow",
		mcp.WithDescription("Create a new automation workflow."),
		mcp.WithString("name", mcp.Required(), mcp.Description("Workflow name")),
		mcp.WithString("steps_json", mcp.Required(), mcp.Description("JSON array of workflow steps")),
	), ns.handleCreateWorkflow)

	// Tool: negotiate_list_workflows
	ns.mcpServer.AddTool(mcp.NewTool("negotiate_list_workflows",
		mcp.WithDescription("List all automation workflows."),
	), ns.handleListWorkflows)

	// Tool: negotiate_run_workflow
	ns.mcpServer.AddTool(mcp.NewTool("negotiate_run_workflow",
		mcp.WithDescription("Execute a workflow and log the result."),
		mcp.WithInteger("workflow_id", mcp.Required(), mcp.Description("Workflow ID to run")),
		mcp.WithString("params", mcp.Description("Optional JSON params to pass to the workflow")),
	), ns.handleRunWorkflow)

	// Tool: negotiate_workflow_log
	ns.mcpServer.AddTool(mcp.NewTool("negotiate_workflow_log",
		mcp.WithDescription("View execution log for a workflow."),
		mcp.WithInteger("workflow_id", mcp.Required(), mcp.Description("Workflow ID")),
	), ns.handleWorkflowLog)

	// Tool: negotiate_batch_upload
	ns.mcpServer.AddTool(mcp.NewTool("negotiate_batch_upload",
		mcp.WithDescription("Upload a CSV file to create multiple negotiations at once. The CSV must have a header row: vendor,strategy,budget,target_price,notes."),
		mcp.WithString("csv_content", mcp.Required(), mcp.Description("CSV content with header row and data rows")),
	), ns.handleBatchUpload)

		// Tool: negotiate_register_device
		ns.mcpServer.AddTool(mcp.NewTool("negotiate_register_device",
			mcp.WithDescription("Register a mobile device for push notifications"),
			mcp.WithString("token", mcp.Required(), mcp.Description("Device push token")),
			mcp.WithString("platform", mcp.Required(), mcp.Description("Platform (ios, android)")),
		), ns.handleRegisterDevice)

		// Tool: negotiate_send_push
		ns.mcpServer.AddTool(mcp.NewTool("negotiate_send_push",
			mcp.WithDescription("Send a push notification to a registered device"),
			mcp.WithNumber("device_id", mcp.Required(), mcp.Description("Device ID to send to")),
		...
		// Tool: negotiate_list_devices
		ns.mcpServer.AddTool(mcp.NewTool("negotiate_list_devices",
        ...
        // ─── P112: Server Config Manager Tools ───
        ns.mcpServer.AddTool(mcp.NewTool("negotiate_get_config",
                mcp.WithDescription("Retrieve all server configuration entries."),
        ), ns.handleGetConfig)

        ns.mcpServer.AddTool(mcp.NewTool("negotiate_set_config",
                mcp.WithDescription("Set a server configuration key-value pair."),
        ...
        ns.mcpServer.AddTool(mcp.NewTool("negotiate_export_config",
                mcp.WithDescription("Export all server configuration entries as JSON."),
        ), ns.handleExportConfig)

        ns.mcpServer.AddTool(mcp.NewTool("negotiate_import_config",
                mcp.WithDescription("Import server configuration entries from JSON."),
                mcp.WithString("json_data", mcp.Required(), mcp.Description("JSON string of configuration entries")),
        ), ns.handleImportConfig)

	// ─── P113: Backup & Restore Tools ───
	ns.mcpServer.AddTool(mcp.NewTool("negotiate_create_backup",
		mcp.WithDescription("Create a new system backup with optional table selection."),
		mcp.WithString("tables", mcp.Description("Comma-separated table names (default: all)")),
	), ns.handleCreateBackup)

	ns.mcpServer.AddTool(mcp.NewTool("negotiate_list_backups",
		mcp.WithDescription("List all system backups ordered by creation time (newest first)."),
	), ns.handleListBackups)

	ns.mcpServer.AddTool(mcp.NewTool("negotiate_restore_backup",
		mcp.WithDescription("Restore a system backup by its ID."),
		mcp.WithNumber("backup_id", mcp.Required(), mcp.Description("Backup ID to restore")),
	), ns.handleRestoreBackup)

	ns.mcpServer.AddTool(mcp.NewTool("negotiate_backup_schedule",
		mcp.WithDescription("Set or update the automated backup schedule."),
		mcp.WithString("cron", mcp.Required(), mcp.Description("Cron expression (e.g., 0 2 * * * for daily at 2am)")),
		mcp.WithString("tables", mcp.Description("Comma-separated table names (default: all)")),
	), ns.handleBackupSchedule)
}

// ─── Tool Handlers ───
func (ns *NegotiationServer) handleRegisterDevice(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	...
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: platform"), nil
	}

	ns.logger.Debug("negotiate_register_device called", "token", token, "platform", platform)

	device, err := ns.pushStore.RegisterDevice(ctx, token, platform)
	if err != nil {
		ns.logger.Warn("negotiate_register_device failed", "error", err.Error())
		return mcp.NewToolResultError("Register device failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleSendPush(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	deviceID, err := req.RequireInt("device_id")
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: device_id"), nil
	}
	...
	ns.logger.Debug("negotiate_send_push called", "device_id", deviceID, "title", title)

	result, err := ns.pushEng.SendPush(ctx, deviceID, title, body)
	if err != nil {
		ns.logger.Warn("negotiate_send_push failed", "error", err.Error())
		return mcp.NewToolResultError("Send push failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleListDevices(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	ns.logger.Debug("negotiate_list_devices called")

	devices, err := ns.pushStore.ListDevices(ctx)
	if err != nil {
		ns.logger.Warn("negotiate_list_devices failed", "error", err.Error())
		return mcp.NewToolResultError("List devices failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleScheduleRun(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	...
	cronExpr, err := req.RequireString("cron_expr")
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: cron_expr"), nil
	}

	ns.logger.Debug("negotiate_schedule_run called", "vendor", vendor, "strategy", strategy, "cron_expr", cronExpr)

	schedule, err := ns.schedulerStore.CreateSchedule(ctx, vendor, strategy, cronExpr)
	if err != nil {
		ns.logger.Warn("negotiate_schedule_run failed", "error", err.Error())
		return mcp.NewToolResultError("Schedule run failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleListSchedules(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	ns.logger.Debug("negotiate_list_schedules called")

	schedules, err := ns.schedulerStore.ListSchedules(ctx)
	if err != nil {
		ns.logger.Warn("negotiate_list_schedules failed", "error", err.Error())
		return mcp.NewToolResultError("List schedules failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleDeleteSchedule(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	scheduleID, err := req.RequireInt("schedule_id")
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: schedule_id"), nil
	}

	ns.logger.Debug("negotiate_delete_schedule called", "schedule_id", scheduleID)

	if err := ns.schedulerStore.DeleteSchedule(ctx, scheduleID); err != nil {
		ns.logger.Warn("negotiate_delete_schedule failed", "error", err.Error())
		return mcp.NewToolResultError("Delete schedule failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleScheduleResults(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	...
	ns.logger.Debug("negotiate_schedule_results called", "schedule_id", scheduleID)

	results, err := ns.schedulerStore.GetScheduleResults(ctx, scheduleID)
	if err != nil {
		ns.logger.Warn("negotiate_schedule_results failed", "error", err.Error())
		return mcp.NewToolResultError("Schedule results failed: " + err.Error()), nil
	}
...
// ─── P103: Workflow Builder Handlers ───

func (ns *NegotiationServer) handleCreateWorkflow(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	...
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: steps_json"), nil
	}

	ns.logger.Debug("negotiate_create_workflow called", "name", name)

	workflow, err := ns.workflowStore.CreateWorkflow(ctx, name, stepsJSON)
	if err != nil {
		ns.logger.Warn("negotiate_create_workflow failed", "error", err.Error())
		return mcp.NewToolResultError("Create workflow failed: " + err.Error()), nil
	}

	resp := map[string]any{
		"id":          workflow.ID,
		"name":        workflow.Name,
		"steps_json":  workflow.StepsJSON,
...
func (ns *NegotiationServer) handleListWorkflows(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	ns.logger.Debug("negotiate_list_workflows called")

	workflows, err := ns.workflowStore.ListWorkflows(ctx)
	if err != nil {
		ns.logger.Warn("negotiate_list_workflows failed", "error", err.Error())
		return mcp.NewToolResultError("List workflows failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleRunWorkflow(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	workflowID, err := req.RequireInt("workflow_id")
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: workflow_id"), nil
	}
	params := req.GetString("params", "{}")

	ns.logger.Debug("negotiate_run_workflow called", "workflow_id", workflowID, "params", params)

	result, err := ns.workflowStore.RunWorkflow(ctx, workflowID, params)
	if err != nil {
		ns.logger.Warn("negotiate_run_workflow failed", "error", err.Error())
		return mcp.NewToolResultError("Run workflow failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleWorkflowLog(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	...
	ns.logger.Debug("negotiate_workflow_log called", "workflow_id", workflowID)

	logs, err := ns.workflowStore.GetWorkflowLog(ctx, workflowID)
	if err != nil {
		ns.logger.Warn("negotiate_workflow_log failed", "error", err.Error())
		return mcp.NewToolResultError("Workflow log failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleBatchUpload(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	csvContent, err := req.RequireString("csv_content")
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: csv_content"), nil
	}

	ns.logger.Debug("negotiate_batch_upload called")

	result, err := ns.batchCsvEng.ProcessCSV(ctx, csvContent)
	if err != nil {
		ns.logger.Warn("negotiate_batch_upload failed", "error", err.Error())
		return mcp.NewToolResultError("Batch upload failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleGetConfig(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	ns.logger.Debug("negotiate_get_config called")

	entries, err := ns.serverCfgStore.GetConfig(ctx)
	if err != nil {
		ns.logger.Warn("negotiate_get_config failed", "error", err.Error())
		return mcp.NewToolResultError("Get config failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleSetConfig(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	...
	ns.logger.Debug("negotiate_set_config called", "key", key)

	if err := ns.serverCfgStore.SetConfig(ctx, key, value); err != nil {
		ns.logger.Warn("negotiate_set_config failed", "error", err.Error())
		return mcp.NewToolResultError("Set config failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleExportConfig(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	ns.logger.Debug("negotiate_export_config called")

	jsonStr, err := ns.serverCfgStore.ExportConfig(ctx)
	if err != nil {
		ns.logger.Warn("negotiate_export_config failed", "error", err.Error())
		return mcp.NewToolResultError("Export config failed: " + err.Error()), nil
	}
...
func (ns *NegotiationServer) handleImportConfig(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()

	jsonData, err := req.RequireString("json_data")
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: json_data"), nil
	}

	ns.logger.Debug("negotiate_import_config called")

	count, err := ns.serverCfgStore.ImportConfig(ctx, jsonData)
	if err != nil {
		ns.logger.Warn("negotiate_import_config failed", "error", err.Error())
		return mcp.NewToolResultError("Import config failed: " + err.Error()), nil
	}
...
// ─── P113: Backup & Restore Handlers ───

func (ns *NegotiationServer) handleCreateBackup(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	tables := req.GetString("tables", "all")
	ns.logger.Debug("negotiate_create_backup called", "tables", tables)
	if ns.backupStore == nil {
		return mcp.NewToolResultError("Backup store not available"), nil
	}
	backup, err := ns.backupStore.CreateBackup(ctx, tables)
	if err != nil {
		return mcp.NewToolResultError("Create backup failed: " + err.Error()), nil
	}
	return ns.jsonResult(map[string]any{"backup": backup, "duration_ms": time.Since(start).Milliseconds()})
}

func (ns *NegotiationServer) handleListBackups(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	ns.logger.Debug("negotiate_list_backups called")
	if ns.backupStore == nil {
		return mcp.NewToolResultError("Backup store not available"), nil
	}
	backups, err := ns.backupStore.ListBackups(ctx)
	if err != nil {
		return mcp.NewToolResultError("List backups failed: " + err.Error()), nil
	}
	return ns.jsonResult(map[string]any{"backups": backups, "count": len(backups), "duration_ms": time.Since(start).Milliseconds()})
}

func (ns *NegotiationServer) handleRestoreBackup(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	id, err := req.RequireInt("backup_id")
	if err != nil {
		return mcp.NewToolResultError("Missing required parameter: backup_id"), nil
	}
	ns.logger.Debug("negotiate_restore_backup called", "backup_id", id)
	if ns.backupStore == nil {
		return mcp.NewToolResultError("Backup store not available"), nil
	}
	backup, err := ns.backupStore.RestoreBackup(ctx, int(id))
	if err != nil {
		return mcp.NewToolResultError("Restore backup failed: " + err.Error()), nil
	}
	return ns.jsonResult(map[string]any{"backup": backup, "duration_ms": time.Since(start).Milliseconds()})
}

func (ns *NegotiationServer) handleBackupSchedule(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	start := time.Now()
	cron, _ := req.RequireString("cron")
	tables := req.GetString("tables", "all")
	ns.logger.Debug("negotiate_backup_schedule called", "cron", cron)
	if ns.backupStore == nil {
		return mcp.NewToolResultError("Backup store not available"), nil
	}
	if err := ns.backupStore.SetBackupSchedule(ctx, cron, tables); err != nil {
		return mcp.NewToolResultError("Set schedule failed: " + err.Error()), nil
	}
	return ns.jsonResult(map[string]any{"status": "ok", "cron": cron, "duration_ms": time.Since(start).Milliseconds()})
}
